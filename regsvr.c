...
static HRESULT register_interfaces(struct regsvr_interface const* list) {
  ...
  for (; res == ERROR_SUCCESS && list->iid; ++list) {
    ...
    res = RegCreateKeyExW(interface_key, buf, 0, NULL, 0, KEY_READ | KEY_WRITE,
                          NULL, &iid_key, NULL);
...
static HRESULT register_coclasses(struct regsvr_coclass const* list) {
  ...
  for (; res == ERROR_SUCCESS && list->clsid; ++list) {
    ...
    res = RegCreateKeyExW(coclass_key, buf, 0, NULL, 0, KEY_READ | KEY_WRITE,
                          NULL, &clsid_key, NULL);
    if (res != ERROR_SUCCESS) goto error_close_coclass_key;

    if (list->name) {
      res = RegSetValueExA(clsid_key, NULL, 0, REG_SZ,
                           (const BYTE*)(list->name), strlen(list->name) + 1);
    ...
    if (list->progid) {
      ...
      res = register_progid(buf, list->progid, NULL, list->name,
                            list->progid_extra);
      if (res != ERROR_SUCCESS) goto error_close_clsid_key;
    }
...
static HRESULT unregister_coclasses(struct regsvr_coclass const* list) {
  ...
  res = RegOpenKeyExW(HKEY_CLASSES_ROOT, clsid_keyname, 0, KEY_READ | KEY_WRITE,
                      &coclass_key);
  if (res == ERROR_FILE_NOT_FOUND) return S_OK;
  if (res != ERROR_SUCCESS) goto error_return;
...
static LONG register_key_defvalueW(HKEY base, WCHAR const* name,
  ...
  res = RegCreateKeyExW(base, name, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL,
                        &key, NULL);
...
static LONG register_progid(WCHAR const* clsid, char const* progid,
  ...
  res = RegCreateKeyExA(HKEY_CLASSES_ROOT, progid, 0, NULL, 0,
                        KEY_READ | KEY_WRITE, NULL, &progid_key, NULL);
  if (res != ERROR_SUCCESS) return res;

  if (name) {
    res = RegSetValueExA(progid_key, NULL, 0, REG_SZ, (const BYTE*)name,
                         strlen(name) + 1);
  ...
  if (extra) {
    ...
    res = RegCreateKeyExA(progid_key, extra, 0, NULL, 0, KEY_READ | KEY_WRITE,
                          NULL, &extra_key, NULL);
    if (res == ERROR_SUCCESS) RegCloseKey(extra_key);
  }
...
static HRESULT register_driver(void) {
  ...
  LPCSTR clsid = "CLSID";
  LPCSTR wine_clsid = CLISID_WineASIOString;
  LPCSTR desc = "Description";
  LPCSTR wine_desc = DRIVER_NAME " Driver";
  HKEY key;
  LONG rc;

  TRACE("register_driver...");

  rc = RegOpenKeyExA(HKEY_LOCAL_MACHINE, asio_key, 0, KEY_READ | KEY_WRITE,
                     &key);

  if (rc != ERROR_SUCCESS)
    rc = RegCreateKeyExA(HKEY_LOCAL_MACHINE, asio_key, 0, NULL, 0,
                         KEY_READ | KEY_WRITE, NULL, &key, 0);
...
HRESULT WINAPI DllRegisterServer(void) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) hr = register_driver();
  }
  return hr;
}
...
HRESULT WINAPI DllUnregisterServer(void) {
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) hr = unregister_driver();
  }
  return hr;
}
